...
fn pbs_tbn_reorthonormalizes_interpolated_frame() -> io::Result<()> {
    let src = fs::read_to_string(manifest_dir().join("shaders/modules/pbs/normal.wgsl"))?;
...
fn custom_mesh_tbn_shaders_route_through_shared_parity() -> io::Result<()> {
    let xiexe =
        fs::read_to_string(manifest_dir().join("shaders/modules/xiexe/toon2/surface.wgsl"))?;
    assert!(
