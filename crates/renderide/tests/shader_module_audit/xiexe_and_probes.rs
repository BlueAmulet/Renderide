...
fn xiexe_matcap_uses_stereo_center_view_dir() -> io::Result<()> {
    let globals_src =
        fs::read_to_string(manifest_dir().join("shaders/modules/frame/globals.wgsl"))?;
    assert!(
    ...
    let lighting_src =
        fs::read_to_string(manifest_dir().join("shaders/modules/xiexe/toon2/lighting.wgsl"))?;
    assert!(
...
fn xiexe_generic_stems_resolve_alpha_mode_from_keywords() -> io::Result<()> {
    let base_src =
        fs::read_to_string(manifest_dir().join("shaders/modules/xiexe/toon2/base.wgsl"))?;
