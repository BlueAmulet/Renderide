...
impl<T> BoundedRing<T> {
    ...
    ///
    /// Used by [`super::DriverThread::submit`] and shutdown logging to report ring saturation
    /// next to submit backlog. Acquires the inner mutex; this is fine because the function runs
    /// once per submit and during teardown, not inside GPU command recording loops.
