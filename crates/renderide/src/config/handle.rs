...
//! the user edits values; saves to disk go through [`super::persist::save`]. Wrapping the
//! settings in `Arc<RwLock<...>>` (rather than handing out clones) means the HUD's edits are
//! immediately visible to the next frame without a propagation step.

use std::sync::Arc;

use super::persist::ConfigLoadResult;
