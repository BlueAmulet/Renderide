    ...
    fn body(&self, ui: &imgui::Ui, ctx: &StatsContext<'_>) {
        ...
        kv_table(ui, "draws_kv", |ui| {
            kv_split3(
                ui,
                "Batches",
                SplitMetric::new(m.batch_total, "total"),
                SplitMetric::new(m.batch_main, "main"),
                SplitMetric::new(m.batch_overlay, "overlay"),
            );
            kv_split3(
                ui,
                "Draws",
                SplitMetric::new(m.draws_total, "total"),
                SplitMetric::new(m.draws_main, "main"),
                SplitMetric::new(m.draws_overlay, "overlay"),
            );
            kv(
                ui,
    ...
    fn body(&self, ui: &imgui::Ui, ctx: &StatsContext<'_>) {
        ...
        kv_table(ui, "health_kv", |ui| {
            ...
            kv_err_or_dim(
                ...
                f.ipc_health.frame_submit_apply_failures.to_string(),
                f.ipc_health.frame_submit_apply_failures,
            );
            kv_err_or_dim(
                ui,
                "OpenXR wait_frame errs",
                f.xr_health.xr_wait_frame_failures.to_string(),
                f.xr_health.xr_wait_frame_failures,
            );
            kv_err_or_dim(
                ui,
                "OpenXR locate_views errs",
                f.xr_health.xr_locate_views_failures.to_string(),
                f.xr_health.xr_locate_views_failures,
            );
            kv_err_or_dim(
                ui,
                "Unhandled IPC cmds",
                f.ipc_health.unhandled_ipc_command_event_total.to_string(),
...
impl<'a> SplitMetric<'a> {
    const fn new(value: usize, label: &'a str) -> Self {
        Self { value, label }
    }
}
