    ...
    fn body(&self, ui: &imgui::Ui, ctx: &StatsContext<'_>) {
        ...
        kv_table(ui, "draws_kv", |ui| {
            kv_split3(
                ui,
                "Batches",
                SplitSegment::new(m.batch_total, "total"),
                SplitSegment::new(m.batch_main, "main"),
                SplitSegment::new(m.batch_overlay, "overlay"),
            );
            kv_split3(
                ui,
                "Draws",
                SplitSegment::new(m.draws_total, "total"),
                SplitSegment::new(m.draws_main, "main"),
                SplitSegment::new(m.draws_overlay, "overlay"),
            );
            kv(
                ui,
...
impl<'a> SplitSegment<'a> {
    fn new(value: usize, label: &'a str) -> Self {
        Self { value, label }
    }
}
