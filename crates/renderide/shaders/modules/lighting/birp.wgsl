...
/// Unity BiRP-style spotlight cone attenuation.
///
/// `l` is the normalized direction from the shaded surface toward the light. The packed light
/// stores the outer half-angle cosine plus a scale that reaches full intensity at the quarter-angle
/// cone, matching Unity's `unity_LightAtten` spot-light fade behavior.
fn spot_angle_attenuation(light: ft::GpuLight, l: vec3<f32>) -> f32 {
    ...
    return clamp((rho - light.spot_cos_half_angle) * light.spot_angle_scale, 0.0, 1.0);
}
